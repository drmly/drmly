...
// screenDream runs python with a random set of params
func (s *ss) screenDream() {
	...
	cmd, err := exec.Command("python3", "file.py", "--input", s.path, "-ch", s.ch, "-it", s.it, "-oc", s.oc, "-la", s.la, "-os", s.os).CombinedOutput()
	if err != nil {
	...
	savedFile := basePath + "/images/" + s.name + "/output/1.png"
			...
			case event := <-w.Event:
				...
				if filetype.IsImage(buf) {
					go func() {
						...
						s.path = fmt.Sprint(basePath + "/images/" + s.name)
